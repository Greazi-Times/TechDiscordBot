...
public class Storage {
    ...
    public void createDefault() {
        ...
        mysql.update("CREATE TABLE IF NOT EXISTS `"+SUBVERIFIED_TABLE+"`(`id` int NOT NULL AUTO_INCREMENT, `subMemberId` int NOT NULL, `verificationId` int NOT NULL, PRIMARY KEY (`id`), KEY `fkIdx_126` (`subMemberId`), CONSTRAINT `FK_124` FOREIGN KEY `fkIdx_126` (`subMemberId`) REFERENCES `Members` (`id`),KEY `fkIdx_129` (`verificationId`), CONSTRAINT `FK_127` FOREIGN KEY `fkIdx_129` (`verificationId`) REFERENCES `Verification` (`id`));");
        mysql.update("CREATE TABLE IF NOT EXISTS `"+TRANSCRIPTS_TABLE+"`(`id` varchar(100) NOT NULL, `value` text NOT NULL, PRIMARY KEY (`id`));");
        mysql.update("CREATE TABLE IF NOT EXISTS `"+PURCHASEDPLUGINS_TABLE+"`(`id` int NOT NULL AUTO_INCREMENT, `marketId` int NOT NULL, `transactionId` varchar(255) NOT NULL, `purchaseData` varchar(255) NOT NULL, `price` FLOAT NOT NULL, `date` bigint(20) NOT NULL, `reviewed` boolean NOT NULL DEFAULT false, `verificationId` int NOT NULL, `resourceId` int NOT NULL, PRIMARY KEY (`id`), KEY `fkIdx_107` (`marketId`), CONSTRAINT `FK_105` FOREIGN KEY `fkIdx_107` (`marketId`) REFERENCES `Markets` (`id`), KEY `fkIdx_110` (`verificationId`), CONSTRAINT `FK_108` FOREIGN KEY `fkIdx_110` (`verificationId`) REFERENCES `Verification` (`id`),KEY `fkIdx_116` (`resourceId`),CONSTRAINT `FK_114` FOREIGN KEY `fkIdx_116` (`resourceId`) REFERENCES `Resources` (`id`));");
    ...
    public VerificationPluginList retrieveVerificationPlugins(@NotNull DbVerification Verification) {
        ...
        try {
            Connection connection = mysql.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM " + PURCHASEDPLUGINS_TABLE + " WHERE id='"+Verification.getId()+"';");
            ResultSet rs = preparedStatement.executeQuery();

            while (rs.next())
                VerificationPluginList.add(new VerificationPlugin(rs.getInt("id"), rs.getInt("marketId"), rs.getInt("verificationId"), rs.getInt("resourceId"), rs.getString("transactionId"), rs.getString("purchaseData"), rs.getFloat("price"), rs.getLong("date"), rs.getBoolean("reviewed")));
